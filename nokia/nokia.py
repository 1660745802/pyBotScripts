...
font = ImageFont.truetype("nokia/fonts/1.ttf", font_size)
...
def generate_image(text: str):
    origin_im = Image.open("nokia/images/3.png")
    text = text[:900]
    length = len(text)
    width, height = font.getsize(text)
    current_width = 0
...
def gen_img(message):
    img = generate_image(message[3:].strip())
    message_chain = [False, {"type": "Image", "base64": str(img, 'utf-8')}]
